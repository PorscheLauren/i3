static void GENERATED_call(const int call_identifier, struct CommandResultIR *result) {
    switch (call_identifier) {
         ...
         case 7:
             result->next_state = INITIAL;
#ifndef TEST_PARSER
             cmd_move_window_to_position(&current_match, result, get_string("method"), get_long("coord_x"), get_long("coord_y"));
#else
           fprintf(stderr, "cmd_move_window_to_position(%s, %ld, %ld)\n", get_string("method"), get_long("coord_x"), get_long("coord_y"));
#endif
             break;
         case 8:
             result->next_state = INITIAL;
         ...
         case 35:
             result->next_state = INITIAL;
#ifndef TEST_PARSER
             cmd_resize_set(&current_match, result, get_long("width"), get_long("height"));
#else
           fprintf(stderr, "cmd_resize_set(%ld, %ld)\n", get_long("width"), get_long("height"));
#endif
             break;
         case 36:
             result->next_state = INITIAL;
         ...
         case 37:
             result->next_state = INITIAL;
#ifndef TEST_PARSER
             cmd_resize(&current_match, result, get_string("way"), get_string("direction"), get_long("resize_px"), 10);
#else
         ...
         case 38:
             result->next_state = INITIAL;
#ifndef TEST_PARSER
             cmd_border(&current_match, result, get_string("border_style"), 2);
#else
